		...
		<div id="list-site" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<th><g:message code="site.bank.label" default="Bank" /></th>
					
						<g:sortableColumn property="balance" title="${message(code: 'site.balance.label', default: 'Balance')}" />
					
						<g:sortableColumn property="balanceDate" title="${message(code: 'site.balanceDate.label', default: 'Balance Date')}" />
				...
				<g:each in="${siteInstanceList}" status="i" var="siteInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${siteInstance.id}">${fieldValue(bean: siteInstance, field: "bank")}</g:link></td>
					
						<td>${fieldValue(bean: siteInstance, field: "balance")}</td>
					
						<td><g:formatDate date="${siteInstance.balanceDate}" /></td>
